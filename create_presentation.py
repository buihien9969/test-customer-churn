...
with open('MCI_Churn_Analysis_Presentation.md', 'r', encoding='utf-8') as f:
    md_content = f.read()

# Split the markdown into slides
...
# Special handling for the model comparison slide (slide 8)
model_slide_index = 7  # 0-based index
if len(prs.slides) > model_slide_index:
    slide = prs.slides[model_slide_index]

    # Add a table for model comparison
    left = Inches(0.5)
    top = Inches(2)
    width = Inches(12)
    height = Inches(3)

    # Create a table with 6 rows (header + 5 models) and 6 columns
    ...
    for row_idx, model_data in enumerate(models, start=1):
        for col_idx, value in enumerate(model_data):
...
# Save the presentation with a new name to avoid permission issues
try:
    prs.save('MCI_Churn_Analysis_Presentation_v2.pptx')
    print("Presentation created successfully with images and formatting!")
